...
asset_path <- "img/blog-images/2018-11-13-antarctic/"
knitr::opts_chunk$set(eval = TRUE, echo = TRUE, message = TRUE, warning = TRUE, tidy = FALSE, cache = FALSE, include = TRUE, dpi = 72, fig.width = 9, fig.height = 6, fig.align = "center", results = "markup", fig.path = asset_path)
```
...
Software development to support Antarctic data usage is growing, but still lags behind the available data, and some common tasks are still more difficult than we would like.
...
1. improving core functionality centred around three key tasks that many researchers find problematic:
...
```{r temp1, echo = FALSE}
## cheaty hide warnings until tz warning bug is fixed in raadtools, see https://github.com/AustralianAntarcticDivision/raadtools/issues/72
suppressWarnings(ele$ice <- extract(readice, ele[, c("lon", "lat", "date")]))
## points outside the ice grid will have missing ice values, so fill them with zeros
ele$ice[is.na(ele$ice)] <- 0
```
```{r extractice, eval = FALSE}
...
```{r plotanim, echo = FALSE, message = FALSE, warning = FALSE, cache = TRUE}
library(SOmap)
...
tail_length <- 8
ele_lagged <- ele
for (li in seq_len(tail_length)) ele_lagged <- rbind(ele_lagged, ele %>% mutate(date = lead(date, li)))
...
g <- ggplot() + geom_raster(data = cx, aes(x, y, fill = z)) +
    ...
    geom_path(data = ele_lagged, aes(x, y), colour = "orange", size = 1) +
