...
knitr::opts_chunk$set(eval = TRUE, echo = TRUE, message = TRUE, warning = TRUE, tidy = FALSE, cache = TRUE, include = TRUE, dpi = 72, fig.width = 9, fig.height = 6, fig.align = "center", results = "markup")
```
...
req <- c("dplyr", "ggplot2", "bsam", "remotes")
req <- req[sapply(req, requireNamespace, quietly = TRUE)]
if (length(req) > 0) install.packages(req)

## and some github packages
req <- c("AustralianAntarcticDivision/blueant", "AustralianAntarcticDivision/raadtools")
req <- req[sapply(basename(req), requireNamespace, quietly = TRUE)]
